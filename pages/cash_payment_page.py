...
class CashPaymentPage(ctk.CTkFrame):
    ...
    def __init__(self, master, controller):
        ...
        self.shell = AppShell(self, title_right=config.get("cash_payment_page", "header_title", default="Cash Payment"))
        ...
        self.helper_text = ctk.CTkLabel(
            ...
            text=config.get(
                "cash_payment_page",
                "helper_text",
                default="Insert bills one at a time and wait for the amount to update."
            ),
    ...
    def _accepted_bills_text(self):
        ...
        joined = ", ".join(map(str, accepted))
        prefix = config.get("cash_payment_page", "accepted_bills_prefix", default="Accepted denominations")
        return f"{prefix}: {joined}"

    def _refresh_from_config(self):
        try:
            self.tolerance = float(config.get("cash_payment_page", "pulse_tolerance_seconds", default=0.30))
            self.back_btn.configure(text=config.get("cash_payment_page", "back_button_text", default="Back"))
            self.page_title.configure(text=config.get("cash_payment_page", "title", default="CASH PAYMENT"))
            self.title_label.configure(text=config.get("cash_payment_page", "title", default="CASH PAYMENT"))
            self.loading_label.configure(text=config.get("cash_payment_page", "loading_text", default="Processing payment..."))
    ...
    def update_data(self, user_data=None, selected_product=None, discount=0, planned_cash_bill=None, **kwargs):
        ...
        if not self.user_data or not self.selected_product:
            ...
            self.order_text.configure(
                text=config.get(
                    ...
                    default="No product selected or user data missing"
                ),
        ...
        planned_line = (
            f"{config.get('cash_payment_page', 'planned_bill_label', default='Planned bill')}: ₱{float(self.planned_cash_bill):.2f}\n"
            if self.planned_cash_bill is not None else ""
        )

        self.order_text.configure(
            text=(
                f"{config.get('cash_payment_page', 'summary_user_label', default='User')}: {self.user_data.get('username', 'User')}\n"
                f"{config.get('cash_payment_page', 'summary_product_label', default='Product')}: {self.selected_product.get('name', 'Unknown')}\n"
                f"{config.get('cash_payment_page', 'summary_type_label', default='Type')}: {self.selected_product.get('type', 'Unknown')}\n"
                f"{config.get('cash_payment_page', 'summary_total_label', default='Total')}: ₱{total:.2f}\n"
                f"{planned_line}"
            ).strip(),
            text_color=theme.BLACK
        )
        ...
        self._set_progress(text=self._accepted_bills_text(), color=theme.MUTED, visible=True)
    ...
    def _set_status(self, text="", color=None, visible=True):
        ...
        self.status_text.configure(text=text, text_color=color or theme.INFO)
    ...
    def _set_progress(self, text="", color=None, visible=True):
        ...
        self.progress.configure(text=text, text_color=color or theme.MUTED)
        self.progress.grid()
    ...
    def _set_helper(self, text="", color=None, visible=True):
        ...
        self.helper_text.configure(text=text, text_color=color or theme.MUTED)
                    ...
                    if bill_value:
                        ...
                        self.controller.after(
                            ...
                            lambda c=stable_count: self._set_status(
                                text=f"{config.get('cash_payment_page', 'unknown_bill_prefix', default='Unknown bill')} ({c} pulses). {config.get('cash_payment_page', 'rejecting_suffix', default='Rejecting...')}",
                                color=theme.ERROR,
                                visible=True
                            )
                        )
    ...
    def process_bill(self, bill_value):
        ...
        if not self.selected_product:
            ...
            self._set_status(
                text=config.get(
                    "cash_payment_page",
                    "no_selection_returning_text",
                    default="No product selected - returning bill"
                ),
                color=theme.ERROR,
                visible=True
            )
        ...
        if remaining > 0:
            self._set_status(
                text=(
                    f"{config.get('cash_payment_page', 'inserted_total_prefix', default='Total inserted')}: "
                    f"₱{self.total_cash_inserted:.2f}. "
                    f"{config.get('cash_payment_page', 'insert_more_prefix', default='Insert')} ₱{remaining:.2f} "
                    f"{config.get('cash_payment_page', 'insert_more_suffix', default='more')}"
                ),
                color=theme.INFO,
                visible=True
            )
            ...
            if not self.transaction_in_progress:
                ...
                self._set_status(
                    text=(
                        f"{config.get('cash_payment_page', 'inserted_total_prefix', default='Total inserted')}: "
                        f"₱{self.total_cash_inserted:.2f}. "
                        f"{config.get('cash_payment_page', 'payment_complete_text', default='Payment complete!')}"
                    ),
                    color=theme.SUCCESS,
                    visible=True
                )
                self._set_helper(
                    text=config.get(
                        "cash_payment_page",
                        "saving_transaction_text",
                        default="Saving your transaction..."
                    ),
                    color=theme.MUTED,
                    visible=True
                )
    ...
    def _handle_transaction_success(self, cash, change, total, transaction_id=None):
        ...
        if change > 0:
            self._set_status(
                text=config.get(
                    "cash_payment_page",
                    "transaction_saved_change_text",
                    default="Transaction saved successfully. Preparing your change..."
                ),
                color=theme.SUCCESS,
                visible=True
            )
            self._set_helper(
                text=config.get(
                    "cash_payment_page",
                    "change_wait_text",
                    default="Please wait while the machine dispenses your coins."
                ),
                color=theme.MUTED,
                visible=True
            )
            ...
            self._set_status(
                text=config.get(
                    "cash_payment_page",
                    "transaction_saved_receipt_text",
                    default="Transaction saved successfully. Generating receipt..."
                ),
                color=theme.SUCCESS,
                visible=True
            )
            self._set_helper(
                text=config.get(
                    "cash_payment_page",
                    "receipt_wait_text",
                    default="No change to dispense. Proceeding to receipt."
                ),
                color=theme.MUTED,
                visible=True
            )
    ...
    def _handle_transaction_failure(self, error_message):
        ...
        self._set_status(text=error_message, color=theme.ERROR, visible=True)
        self._set_helper(
            text=config.get(
                "cash_payment_page",
                "transaction_failed_helper_text",
                default="Transaction was not saved. Please contact assistance or try again."
            ),
            color=theme.ERROR,
            visible=True
        )
    ...
    def reset_fields(self, **kwargs):
        ...
        self.start_status_animation(config.get("cash_payment_page", "status_text", default="Insert bills to pay"), theme.INFO)
        self._set_progress(text=self._accepted_bills_text(), color=theme.MUTED, visible=True)
        self._set_helper(
            text=config.get(
                "cash_payment_page",
